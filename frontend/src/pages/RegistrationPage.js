...
function RegistrationPage() {
  ...
  const dummyEventData = {
    ...
    eventImage: "https://images.unsplash.com/photo-1555949963-aa79d0ebc8fb?w=800",
    date: "Apr 2, 2025",
  ...
  useEffect(() => {
    getEventById(id)
      .then(res => {
        // If the API returns real data, use it. Otherwise, use the dummy data if the IDs match.
        if (res.data) {
          setEvent(res.data);
        } else if (id === dummyEventData._id) {
          setEvent(dummyEventData);
        }
      })
      .catch(err => {
        console.error("API failed, loading dummy data...", err);
        // If the backend fails, fallback to the dummy data
        setEvent(dummyEventData);
      })
